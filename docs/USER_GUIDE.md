...
## PART 11 - EMAIL SENDING ACCOUNTS (Emails > Accounts)

- Connect your sender (e.g. hazeljones.cse@gmail.com) with SMTP: host, port, username,
  app password, optional Reply-To. Passwords are encrypted at rest and never shown again.
- **Verify** performs a real SMTP login test; **Send test email** sends a real message to
  any address you type. SMTP errors are shown verbatim so you can fix them.
- Campaigns pick the sender ("Send from"); the default is your first verified account.
- Inbox delivery (deliverability): add SPF, DKIM and DMARC records for your domain -
  exact steps in SETUP_AND_CONFIGURATION.md section 5. No one can guarantee spam placement;
  these records are the technically correct requirement.

## PART 12 - AUTOMATIC NO-REPLY REMINDER (built-in automation)

- When a lead reaches score 50+ (HOT per your scoring rules), the CRM schedules a
  3-day no-reply reminder for the person the lead is assigned to.
- After 3 days, if the lead still has no inbound reply, that person gets one email:
  subject `[BusinessName] Reminder`, short body, signed "Reminder form Ali Nasir ,
  the great software Engineer".
- It never duplicates: one open reminder per lead; it is skipped or cancelled if the lead
  replies, is converted/lost, or the responsible user is deleted. If no verified sending
  account exists yet, the reminder waits (open task) and sends once email is configured.
- You can see these on the Tasks page (type REMINDER).

## PART 13 - USERS-LITE DIRECTORY (why reps can now use pickers)

Assignment, chat, and task pickers use a light org directory (name + email only) that every
role can read. Full user management (roles, status, passwords) stays behind USER_VIEW -
admins and managers only. This is the fix for reps not being able to open leads fully.

## PART 14 - CAMPAIGN RECIPIENTS FROM YOUR LEADS

On a campaign page, "Add recipients" now has two tabs: **From your leads** (search your
CRM leads, tick the ones with email addresses, add them in one click) and **Paste emails**
(same as before). Duplicates and suppressed addresses are skipped automatically either way.

## PART 15 - PIPELINE AT A GLANCE

The Pipeline page now shows the full stage path as numbered chips at the top
(1. New -> 2. Qualification -> ... -> Won/Lost) with live lead counts per stage; clicking a
chip scrolls to that column. Every card shows its owner and the next follow-up date
(amber when overdue), so "what is the next step and who owns it" is always visible.

## PART 16 - NEW-USER CREDENTIAL EMAILS (your exact flow)

Admin > Users > create a user and enter THEIR email, leave the password empty. The CRM:
1. generates a secure temporary password (stored only as a bcrypt hash),
2. emails the welcome + login URL + email + temp password FROM your configured sender
   (the VERIFIED account in Emails > Accounts - e.g. hazeljones.cse@gmail.com - falling
   back to CRM_MAIL_* env vars if no UI account exists) TO the new user's email,
3. forces the user to choose their own password at first login.
If the email cannot be delivered, creation still succeeds and the admin sees the temp
password once in a toast. "Reset password" (Users page) shares the same sender logic.
OAuth is NOT needed anywhere in this flow - SMTP + Gmail app password only.

## PART 17 - CALL HEALTH PANEL & AUDIO TRUTH

Open **Calls** - the top-left card is **Call health** with six live lines:

| Line | What it really checks |
|---|---|
| Microphone | Your browser's mic permission + devices ("Check microphone" runs a real probe) |
| Calling device | YOUR registered phone's heartbeat (ONLINE = bridge app running) |
| Phone bridge | Backend pings the bridge's /status (version, latency, active calls) |
| Android phone (adb) | Does adb actually see the phone right now |
| SIM | `gsm.sim.state` read over adb (READY = can place calls) |
| Call audio | Honest gray line: software cannot measure sound - make a 15s test call and check BOTH directions |

If any line is red, the line itself tells you the fix. Full tuning (Windows mic,
Bluetooth HFP, Phone Link settings) + the 15-scenario verification matrix:
`docs/CALL_AUDIO_TUNING.md`. Remember: audio never passes through this CRM - the
bridge only dials; the SIM call itself is carrier quality.

New-setup or "calling stopped working" checklist: `docs/CONFIGURATION_CHECKLIST.md`.

## PART 18 - LEAD FLOW END (WON = completed)

Ab lead lifecycle ka poora end maujood hai:
NEW -> WORKING -> QUALIFIED/INTERESTED -> (deal banao) -> deal WIN karo -> lead khud
**WON** ho jati hai (timeline me "Lead completed (WON)" entry ke saath). WON lead par
reminder automation band ho jati hai. CONverted lead (client ban gayi) bhi terminal hai.
Lead detail page ke Status dropdown me WON ab select kar sakte hain; reminder sweep WON
leads ko "lead won - flow completed" mark kar deta hai.

...
export default function ExecutiveDashboard() {
  const { user } = useAuth()
  // Insights gating (brief #13): management analytics are REPORT_VIEW only. The server already
  // enforces this - reps landing on /dashboard get their personal view instead of a wall of 403s.
  const canSeeInsights = useCan('REPORT_VIEW')
  const { data: exec, isLoading, isError, error } = useQuery({ queryKey: ['dashboard', 'executive'], queryFn: async () => (await api.get('/dashboard/executive')).data as Record<string, never>, enabled: canSeeInsights })
  const { data: charts } = useQuery({ queryKey: ['dashboard', 'charts'], queryFn: async () => (await api.get('/dashboard/charts')).data as Record<string, never>, enabled: canSeeInsights })

  if (!canSeeInsights) return <MyDayPage />
  if (isError) {
        return (

...
/** Never trust the wire: null/missing/error-body responses must render as empty, not crash. */
function asTable(data: unknown): ReportTable | null {
  if (!data || typeof data !== 'object') return null
  const d = data as Partial<ReportTable>
...
export default function ReportsPage() {
  ...
  const { data: rawData, isFetching, isError, refetch } = useQuery({
    ...
    <div>
      ...
      <Card>
        ...
        <CardBody className="!px-0">
          {isError ? (
            <EmptyState icon={<BarChart3 className="h-6 w-6" />} title="Report could not be loaded"
              subtitle="The server returned an error. Your session may have expired, or the report service is unavailable."

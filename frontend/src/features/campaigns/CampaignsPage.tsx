...
export default function CampaignsPage() {
  ...
  // GET /email-templates returns a bare array (NOT a PageResponse) - see EmailComposer note.
  const { data: templates } = useQuery({ queryKey: ['email-templates'], queryFn: async () => (await api.get<TemplateItem[]>('/email-templates', { params: { size: 100 } })).data })
    ...
    <div>
      ...
      <div className="card">
        <DataTable data={data} loading={isFetching} columns={columns} onPageChange={setPage} onRowClick={(c) => navigate(`/campaigns/${c.id}`)}
          empty={{ icon: <Megaphone className="h-6 w-6" />, title: 'No campaigns', subtitle: 'A campaign is simple: (1) name it, (2) pick the sender, (3) write/pick the email, (4) add recipient leads, (5) start. Sent/failed/pending results are tracked on the campaign page.' }} />
      </div>

      <Dialog open={createOpen} onClose={() => setCreateOpen(false)} title="New campaign" description="Three quick steps: basics, sender, first email. Recipients are added on the next screen.">
        <div className="space-y-3">
          <p className="text-xs font-semibold uppercase tracking-wide text-slate-400">Step 1 - Basics</p>
          <div>
          ...
          <p className="pt-1 text-xs font-semibold uppercase tracking-wide text-slate-400">Step 2 - Sender &amp; first email</p>
          <div className="grid grid-cols-2 gap-3">
            <div>
              <Label>Send from</Label>
              <Select value={form.accountId} onChange={(e) => setForm({ ...form, accountId: e.target.value })}>
                <option value="">Default (first verified account)</option>
          ...
          {(!accounts || accounts.length === 0) && (
            <p className="text-xs text-amber-600">No sending account yet - connect one under Emails &gt; Accounts first.</p>
          )}
          <p className="rounded-lg bg-slate-50 px-3 py-2 text-xs text-slate-500">
            Next: on the campaign page click <span className="font-medium">Add recipients</span> (lead emails from your CRM) and start the sequence. Suppressed addresses are skipped automatically; sent/failed/pending counts update live.
          </p>
          <p className="rounded-lg bg-slate-50 px-3 py-2 text-xs text-slate-500">

...
import type { CampaignItem, LeadItem, PageResponse } from '@/types'
...
const cnTab = (active: boolean) =>
  cn('rounded-full px-3 py-1 text-xs font-medium', active ? 'bg-blue-600 text-white' : 'bg-slate-100 text-slate-600 hover:bg-slate-200')
...
export default function CampaignDetailPage() {
  ...
  const [source, setSource] = useState<'leads' | 'paste'>('leads')
  const [leadSearch, setLeadSearch] = useState('')
  const [pickedLeads, setPickedLeads] = useState<Set<string>>(new Set())

  const leadPicker = useQuery({
    queryKey: ['campaign-lead-picker', leadSearch],
    queryFn: async () => (await api.get<PageResponse<LeadItem>>('/leads', { params: { q: leadSearch || undefined, size: 100, sort: 'createdAt,desc' } })).data.content,
    enabled: addOpen && source === 'leads',
  })
  ...
  const addRecipients = useMutation({
    mutationFn: async () => {
      const emails = source === 'leads'
        ? (leadPicker.data ?? []).filter((l) => pickedLeads.has(l.id) && l.email).map((l) => l.email as string)
    ...
    onSuccess: (res) => {
      ...
      setPickedLeads(new Set()); setEmailsText('')
  ...
  return (
    <div>
      ...
      <Dialog open={addOpen} onClose={() => setAddOpen(false)} title="Add recipients" description="Pick leads from your CRM, or paste email addresses. Duplicates and suppressed addresses are skipped.">
        <div className="mb-3 flex gap-2">
          <button onClick={() => setSource('leads')}
            className={cnTab(source === 'leads')}>From your leads</button>
          <button onClick={() => setSource('paste')}
            className={cnTab(source === 'paste')}>Paste emails</button>
        </div>

        {source === 'leads' ? (
          <div className="space-y-2">
            <Input value={leadSearch} onChange={(e) => setLeadSearch(e.target.value)} placeholder="Search leads by business name or email..." />
            <div className="max-h-64 space-y-1 overflow-y-auto rounded-lg border border-slate-200 p-2">
              {(leadPicker.data ?? []).filter((l) => l.email).map((l) => (
                <label key={l.id} className="flex cursor-pointer items-center gap-2 rounded px-1.5 py-1 text-sm hover:bg-slate-50">
                  <input type="checkbox" className="h-3.5 w-3.5 rounded border-slate-300 text-blue-600"
                    checked={pickedLeads.has(l.id)}
                    onChange={(e) => {
                      const next = new Set(pickedLeads)
                      if (e.target.checked) next.add(l.id); else next.delete(l.id)
                      setPickedLeads(next)
                    }} />
                  <span className="min-w-0 flex-1 truncate text-slate-700">{l.businessName}</span>
                  <span className="text-xs text-slate-400">{l.email}</span>
                </label>
              ))}
              {(leadPicker.data ?? []).filter((l) => l.email).length === 0 && (
                <p className="py-6 text-center text-xs text-slate-400">
                  {leadPicker.isFetching ? 'Searching...' : 'No leads with an email address match. Leads need an email to receive a campaign.'}
                </p>
              )}
            </div>
            <p className="text-xs text-slate-400">{pickedLeads.size} lead(s) selected</p>
          </div>
        ) : (
        ...
        <div className="mt-4 flex justify-end gap-2">
          ...
          <Button
            disabled={source === 'leads' ? pickedLeads.size === 0 : !emailsText.trim()}
            loading={addRecipients.isPending}
            onClick={() => addRecipients.mutate()}>
            Add {source === 'leads' ? `${pickedLeads.size} lead(s)` : 'recipients'}
          </Button>
        </div>
      </Dialog>
    </div>

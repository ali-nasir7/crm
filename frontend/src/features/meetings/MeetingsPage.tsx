...
const LINK_ERROR = 'Meeting link must be a valid http(s) URL (Google Meet, Zoom, Teams, or any custom link)'
...
export default function MeetingsPage() {
  ...
  const columns: Column<MeetingItem>[] = [
    ...
    { key: 'link', header: '', render: (m) => validLink(m.meetingLink ?? '') && m.meetingLink
      ? <a href={m.meetingLink} target="_blank" rel="noreferrer" title={`Open the exact meeting URL: ${m.meetingLink}`} className="inline-flex items-center gap-1 text-xs font-medium text-blue-600 hover:underline" onClick={(e) => e.stopPropagation()}><ExternalLink className="h-3 w-3" /> Join</a>
      : m.meetingLink
        ? <span className="text-[11px] text-amber-600" title={LINK_ERROR}>Invalid link</span>
        : <span className="text-[11px] text-slate-400" title="No meeting link was added to this meeting">No link</span> },
    {

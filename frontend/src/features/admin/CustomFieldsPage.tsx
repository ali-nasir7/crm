...
export default function CustomFieldsPage() {
  ...
  const { data: fields, isLoading, isError, error: qError, refetch } = useQuery({ queryKey: ['custom-fields'], queryFn: async () => (await api.get<CustomFieldItem[]>('/custom-fields')).data })
            ...
            <tbody>
              ...
              {isError && (
                <tr><td colSpan={6} className="px-5 py-10 text-center">
                  <p className="text-sm font-medium text-slate-800">Unable to load custom fields. Please try again.</p>
                  <p className="mt-0.5 text-xs text-slate-500">{qError instanceof Error ? qError.message : ''}</p>

...
export default function TemplatesPage() {
  ...
  // GET /email-templates returns a bare array (NOT a PageResponse). The page keeps a `page`
  // state only as a refetch key; the full list is mapped into the DataTable contract below.
  const { data, isFetching, isError, error: qError, refetch } = useQuery({
    queryKey: ['email-templates', page],
    queryFn: async () => (await api.get<TemplateItem[]>('/email-templates')).data,
  })
  const list: TemplateItem[] = Array.isArray(data) ? data : (data as unknown as PageResponse<TemplateItem>)?.content ?? []
  const tableData: PageResponse<TemplateItem> | undefined = data
    ? { content: list, page: 0, size: list.length, totalElements: list.length, totalPages: 1 }
    : undefined
  ...
  return (
    <div>
      ...
      />
      <div className="card">
        <DataTable data={tableData} loading={isFetching} columns={columns} onPageChange={setPage}
          error={isError ? (qError instanceof Error ? qError.message : 'Unable to load email templates. Please try again.') : null}
          onRetry={() => refetch()}

...
export default function AccountsPage() {
  ...
  const [form, setForm] = useState({ provider: 'SMTP', email: '', displayName: '', replyTo: '', smtpHost: '', smtpPort: '587', smtpUsername: '', smtpPassword: '', dailyLimit: '500' })
  const [testTo, setTestTo] = useState<{ accountId: string; email: string; to: string } | null>(null)
  ...
  const create = useMutation({
    mutationFn: async () => {
      const body: Record<string, unknown> = { provider: form.provider, email: form.email, displayName: form.displayName || null, replyTo: form.replyTo || null, dailyLimit: parseInt(form.dailyLimit, 10) || 500 }
  ...
  const testSend = useMutation({
    mutationFn: async () => api.post(`/email-accounts/${testTo!.accountId}/test-send`, { to: testTo!.to }),
    onSuccess: () => { toast.push('success', 'Test email sent', `Check the inbox of ${testTo!.to} (and the spam folder).`); setTestTo(null) },
    onError: (e) => toast.push('error', 'Test email failed', apiError(e).message),
  })
  ...
  return (
    <div>
            ...
            <Card key={a.id}>
              <CardBody className="flex items-start justify-between gap-3">
                ...
                <div className="flex gap-1">
                  <Button variant="ghost" size="icon" title="Verify connection (real SMTP login test)" loading={verify.isPending && verify.variables === a.id} onClick={() => verify.mutate(a.id)}><BadgeCheck className="h-4 w-4" /></Button>
                  <Button variant="ghost" size="icon" title="Send a test email" onClick={() => setTestTo({ accountId: a.id, email: a.email, to: '' })}><Mail className="h-4 w-4 text-blue-500" /></Button>
      ...
      <Dialog open={!!testTo} onClose={() => setTestTo(null)} title={`Send test email from ${testTo?.email ?? ''}`}
        description="A real email is sent through this account's SMTP settings. If it lands in spam, see SETUP_AND_CONFIGURATION.md (SPF/DKIM/DMARC).">
        <div className="space-y-3">
          <div>
            <Label required>Send test to</Label>
            <Input type="email" value={testTo?.to ?? ''} onChange={(e) => setTestTo((t) => (t ? { ...t, to: e.target.value } : t))} placeholder="you@yourdomain.com" />
          </div>
        </div>
        <div className="mt-5 flex justify-end gap-2">
          <Button variant="secondary" onClick={() => setTestTo(null)}>Cancel</Button>
          <Button loading={testSend.isPending} disabled={!testTo?.to.trim()} onClick={() => testSend.mutate()}>Send test</Button>
        </div>
      </Dialog>

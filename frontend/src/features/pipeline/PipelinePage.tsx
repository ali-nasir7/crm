...
export default function PipelinePage() {
  ...
  return (
    <div>
      <PageHeader
        title="Pipeline"
        subtitle="Columns flow left to right toward Won. Drag a card to the next stage (every move is recorded); click a card to open the lead."
        actions={
      ...
      {/* Whole path at a glance: stage order + where leads sit right now */}
      <div className="card mb-3 flex flex-wrap items-center gap-1.5 p-3 text-xs">
        {(active?.stages ?? []).map((stage, i) => {
          const n = byStage.get(stage.id)?.length ?? 0
          return (
            <span key={stage.id} className="flex items-center gap-1.5">
              {i > 0 && <span className="text-slate-300">{'->'}</span>}
              <button onClick={() => document.getElementById(`stage-col-${stage.id}`)?.scrollIntoView({ behavior: 'smooth', inline: 'center', block: 'nearest' })}
                className={cn('rounded-full px-2.5 py-1 font-medium ring-1',
                  stage.type === 'WON' ? 'bg-emerald-50 text-emerald-700 ring-emerald-200'
                  : stage.type === 'LOST' ? 'bg-red-50 text-red-600 ring-red-200'
                  : 'bg-slate-50 text-slate-600 ring-slate-200')}>
                {i + 1}. {stage.name} <span className="tabular-nums opacity-60">({n})</span>
              </button>
            </span>
          )
        })}
        <span className="ml-auto text-slate-400">{leads?.totalElements ?? 0} leads in this pipeline</span>
      </div>
      ...
      {active ? (
              ...
              >
                <div className="flex items-center justify-between px-3 py-2.5">
                  <div className="flex items-center gap-2">
                    <span className="flex h-5 w-5 items-center justify-center rounded-full bg-white text-[10px] font-bold text-slate-500 ring-1 ring-slate-200">{active.stages.indexOf(stage) + 1}</span>
                ...
                <div className="min-h-24 space-y-2 px-2 pb-2">
                  ...
                  {items.map((l) => (
                    ...
                    >
                      ...
                      {(l.assignedUserName || l.nextFollowUpAt) && (
                        <div className="mt-1 flex items-center justify-between gap-1 border-t border-slate-100 pt-1">
                          <span className="truncate text-[10px] text-slate-400" title={`Owner: ${l.assignedUserName ?? 'unassigned'}`}>
                            {l.assignedUserName ?? 'Unassigned'}
                          </span>
                          <span className={cn('text-[10px]', l.nextFollowUpAt && new Date(l.nextFollowUpAt) < new Date() ? 'font-medium text-amber-600' : 'text-slate-400')}>
                            {l.nextFollowUpAt ? `Follow-up ${fmtAgo(l.nextFollowUpAt)}` : 'No follow-up set'}
                          </span>
                        </div>
                      )}
                    </div>
        ...
        <div className="card p-10 text-center text-sm text-slate-400">No pipelines configured yet - create one under Admin &gt; Pipelines.</div>
      )}
    </div>
  )
}

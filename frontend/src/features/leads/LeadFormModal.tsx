...
import type { CustomFieldItem, LeadItem, PipelineItem, SourceItem, TagItem, PageResponse , LiteUserItem} from '@/types'
...
export function LeadFormModal({ open, onClose, lead, onSaved }: {
  ...
  const { data: users } = useQuery({ queryKey: ['users-lite'], queryFn: async () => (await api.get<LiteUserItem[]>('/users/lite')).data, enabled: open })

...
import type { ActivityItem, LeadItem, MeetingItem, PageResponse, TaskItem, EmailItem, DocumentItem, ProposalItem, DealItem, CallItem, LiteUserItem} from '@/types'
...
export default function LeadDetailPage() {
  ...
  // GET /documents returns a bare array (NOT a PageResponse) - normalize for the tab count + panel.
  const { data: documentsRaw } = useQuery({
    queryKey: ['lead-documents', id],
    queryFn: async () => (await api.get<DocumentItem[] | PageResponse<DocumentItem>>('/documents', { params: { leadId: id, size: 50 } })).data,
    enabled: !!id,
  })
  const documents: DocumentItem[] = Array.isArray(documentsRaw) ? documentsRaw : (documentsRaw?.content ?? [])
  ...
  const { data: users } = useQuery({ queryKey: ['users-lite'], queryFn: async () => (await api.get<LiteUserItem[]>('/users/lite')).data })
              ...
              <CardBody className="grid grid-cols-1 gap-3">
                ...
                <div>
                  ...
                  <Select value={lead.status} onChange={(e) => patch.mutate({ status: e.target.value })}>
                    {['NEW', 'WORKING', 'NURTURE', 'QUALIFIED', 'INTERESTED', 'WON', 'UNQUALIFIED', 'CONVERTED'].map((s) => <option key={s} value={s}>{s}</option>)}
                  </Select>
                </div>
                <div>
...
export function DocumentsTab({ docId, documents }: { docId: string; documents: DocumentItem[] }) {
  const qc = useQueryClient()
  const toast = useToast()

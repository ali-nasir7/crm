...
public class ProposalService {
    ...
    /** Hard delete. ACCEPTED proposals are protected (commercial record) - reject first.
     *  Everything else deletes with the items and an audit entry. */
    @Transactional
    public void delete(UUID orgId, UUID id) {
        Proposal p = find(orgId, id);
        if ("ACCEPTED".equals(p.getStatus())) {
            throw ApiException.business("Accepted proposals cannot be deleted. Reject it first.");
        }
        items.deleteAll(items.findByProposalIdOrderByPositionAsc(p.getId()));
        proposals.delete(p);
        audit.log("PROPOSAL_DELETE", "PROPOSAL", p.getId(), p.getProposalNumber(), Map.of("status", p.getStatus()), null);
    }

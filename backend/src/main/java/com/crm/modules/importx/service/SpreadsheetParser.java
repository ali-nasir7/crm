...
public class SpreadsheetParser {
    ...
    private Parsed parseCsv(byte[] content) {
        try {
            ...
            // UTF-8 BOM would otherwise glue itself to the first header (breaks alias matching).
            if (!text.isEmpty() && text.charAt(0) == '\uFEFF') text = text.substring(1);
            // Sniff delimiter: Excel exports semicolon/tab CSVs in many locales.
            String firstLine = text.lines().filter(l -> !l.isBlank()).findFirst().orElse("");
            char delimiter = sniffDelimiter(firstLine);
            Iterable<org.apache.commons.csv.CSVRecord> records = org.apache.commons.csv.CSVFormat.DEFAULT
                .builder().setDelimiter(delimiter).setIgnoreEmptyLines(true).setTrim(true).build()
    ...
    /** Picks the delimiter that appears most often in the header line (comma wins ties). */
    private char sniffDelimiter(String headerLine) {
        char best = ',';
        long bestCount = headerLine.chars().filter(c -> c == ',').count();
        long semi = headerLine.chars().filter(c -> c == ';').count();
        long tab = headerLine.chars().filter(c -> c == '\t').count();
        if (semi > bestCount) { best = ';'; bestCount = semi; }
        if (tab > bestCount) { best = '\t'; }
        return best;
    }

...
public interface EmailAccountRepository extends JpaRepository<EmailAccount, UUID> {
    ...
    /** The org's default sender: first VERIFIED SMTP account (creation order). */
    Optional<EmailAccount> findFirstByOrganizationIdAndProviderAndStatusOrderByCreatedAtAsc(
        UUID organizationId, EmailAccount.Provider provider, String status);
}

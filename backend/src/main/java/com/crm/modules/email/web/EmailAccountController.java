...
public class EmailAccountController {
    ...
    @PreAuthorize("hasAuthority('" + PermissionKeys.EMAIL_ACCOUNT_UPDATE + "')")
    public java.util.Map<String, String> testSend(@PathVariable UUID id, @Valid @RequestBody TestSendRequest request) {
        emailService.sendTestEmail(CurrentUser.require().getOrganizationId(), CurrentUser.require().getId(),
            id, request.to());

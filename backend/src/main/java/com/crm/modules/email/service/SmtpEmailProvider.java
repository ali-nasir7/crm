...
public class SmtpEmailProvider implements EmailProvider {
    ...
    public String send(SendCommand cmd) throws Exception {
        ...
        if (cmd.replyTo() != null && !cmd.replyTo().isBlank()) {
            message.setReplyTo(new InternetAddress[]{new InternetAddress(cmd.replyTo())});
        }
    ...
    /** Real connection probe: authenticates against the SMTP server without sending a message. */
    public void verify(String host, Integer port, String encryption, String username, String password) throws Exception {
        if (host == null || host.isBlank()) throw new Exception("SMTP host is required");
        Properties props = new Properties();
        props.put("mail.smtp.auth", "true");
        props.put("mail.smtp.host", host);
        props.put("mail.smtp.port", String.valueOf(port == null ? 587 : port));
        props.put("mail.smtp.connectiontimeout", "8000");
        props.put("mail.smtp.timeout", "12000");
        switch (encryption == null ? "STARTTLS" : encryption.toUpperCase()) {
            case "SSL", "SSL_TLS" -> props.put("mail.smtp.ssl.enable", "true");
            case "NONE" -> props.put("mail.smtp.auth", password == null || password.isBlank() ? "false" : "true");
            default -> props.put("mail.smtp.starttls.enable", "true");
        }
        Session session = Session.getInstance(props);
        try (Transport transport = session.getTransport("smtp")) {
            transport.connect(host, port == null ? 587 : port, username, password);
        }
    }

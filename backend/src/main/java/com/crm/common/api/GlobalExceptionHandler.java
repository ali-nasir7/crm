...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ErrorResponse> handleIllegalState(IllegalStateException ex, HttpServletRequest req) {
        // Environmental failures (e.g. local storage folder not writable) must say WHAT is wrong,
        // not a bare 500. Real cause stays in the server log with the trace id.
        log.error("Illegal state on {} {}: {}", req.getMethod(), req.getRequestURI(), ex.getMessage());
        String msg = ex.getMessage() == null || ex.getMessage().isBlank()
            ? "Server-side configuration/storage problem. Check the server log (trace id in the response)."
            : ex.getMessage();
        return build(HttpStatus.INTERNAL_SERVER_ERROR, "SERVER_STATE", msg, null, req);
    }

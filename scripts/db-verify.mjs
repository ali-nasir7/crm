...
async function main() {
  ...
  // --- V11: email reply-to (deliverability) ---
  console.log('\n== V11 (email deliverability) ==');
  const v11Cols = await client.query(`SELECT column_name FROM information_schema.columns WHERE table_name='email_accounts' AND column_name='reply_to'`);
  ok('email_accounts: reply_to', v11Cols.rows.length === 1, v11Cols.rows.length ? 'present' : 'missing');

  // --- V12: email_templates soft delete + role import grants ---
  console.log('\n== V12 (templates soft delete + role import) ==');
  const v12a = await client.query(`SELECT column_name FROM information_schema.columns WHERE table_name='email_templates' AND column_name='deleted_at'`);
  ok('email_templates: deleted_at (Hibernate @SQLRestriction)', v12a.rows.length === 1, v12a.rows.length ? 'present' : 'missing');
  // Roles are created at runtime by the DataSeeder, so on a fresh blank DB there are
  // none yet - the correct assertion is "no system role is MISSING the grant".
  const v12b = await client.query(`SELECT count(*)::int AS n FROM roles r WHERE r.is_system AND r.key IN ('SALES_MANAGER','SALES_REP') AND NOT EXISTS (SELECT 1 FROM role_permissions rp JOIN permissions p ON p.id = rp.permission_id WHERE rp.role_id = r.id AND p.key = 'LEAD_IMPORT')`);
  ok('no MANAGER/REP missing LEAD_IMPORT', v12b.rows[0].n === 0, `missing: ${v12b.rows[0].n}`);
  const v12c = await client.query(`SELECT count(*)::int AS n FROM roles r WHERE r.is_system AND r.key = 'SALES_REP' AND NOT EXISTS (SELECT 1 FROM role_permissions rp JOIN permissions p ON p.id = rp.permission_id WHERE rp.role_id = r.id AND p.key = 'IMPORT_VIEW')`);
  ok('no REP missing IMPORT_VIEW', v12c.rows[0].n === 0, `missing: ${v12c.rows[0].n}`);

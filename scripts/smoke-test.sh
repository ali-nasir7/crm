...
# Import must also work for MANAGER and SALES_REP (V12 role grants) - not admin-only.
cat > "$TMP/leads-rep.csv" <<'CSV'
business_name,email
Rep Imported Co,repimported@example.com
CSV
req_upload "/imports" "$MANAGER_TOKEN" -F "file=@$TMP/leads-rep.csv;filename=leads-rep.csv"
chk "MANAGER can upload import" "200 201" '"id"'
IMP_M="$(jstr "$BODY" id)"
req PUT "/imports/$IMP_M/mapping" '{"mapping":{"business_name":"businessName","email":"email"},"duplicateStrategy":"SKIP","options":{}}' "$MANAGER_TOKEN"
chk "MANAGER can submit mapping" "200 202"
req_upload "/imports" "$REP_TOKEN" -F "file=@$TMP/leads-rep.csv;filename=leads-rep.csv"
chk "SALES_REP can upload import" "200 201" '"id"'
IMP_R="$(jstr "$BODY" id)"
req PUT "/imports/$IMP_R/mapping" '{"mapping":{"business_name":"businessName","email":"email"},"duplicateStrategy":"SKIP","options":{}}' "$REP_TOKEN"
chk "SALES_REP can submit mapping" "200 202"
req GET "/imports?page=0&size=5" "$REP_TOKEN"; chk "SALES_REP sees import history (IMPORT_VIEW)" "200" '"content"'
...
chk "sendEmail=true -> emailed (200) or honest 422 without any mail config" "200 422"
...
# =============================================================================
hdr "23. Rep access: users-lite org directory + email test-send authz"
req GET /users/lite "" "$REP_TOKEN"
chk "rep CAN fetch org user directory (users/lite)" "200" "displayName"
req GET /users "" "$REP_TOKEN"
chk "rep still CANNOT list full users (403)" "403"
req POST /email-accounts/00000000-0000-0000-0000-000000000000/test-send '{"to":"someone@example.com"}' "$REP_TOKEN"
chk "rep CANNOT trigger email test-send (403)" "403"
req POST /email-accounts/00000000-0000-0000-0000-000000000000/test-send '{"to":"someone@example.com"}' "$ADMIN_TOKEN"
chk "admin test-send on missing account -> 404" "404"
req GET "/audit-logs?size=5" "$ADMIN_TOKEN"
chk "audit list loads (labels enrichment)" "200"

# =============================================================================
hdr "24. Reminder automation: HOT lead (score 50+) schedules 3-day no-reply reminder"
HOT_NAME="Hot Lead $SUFFIX"
req POST /leads "{\"businessName\":\"$HOT_NAME\",\"email\":\"hotlead$SUFFIX@example.com\"}" "$MANAGER_TOKEN"
chk "manager creates lead" "200 201"
HOT_LEAD="$(jstr "$BODY" id)"
req PUT "/leads/$HOT_LEAD" '{"score":80}' "$MANAGER_TOKEN"
chk "lead score raised to 80 (HOT)" "200"
req GET "/tasks?leadId=$HOT_LEAD&size=50" "$MANAGER_TOKEN"
chk "REMINDER task auto-created for HOT lead" "200" "REMINDER"
req PUT "/leads/$HOT_LEAD" '{"score":10}' "$MANAGER_TOKEN"
chk "score lowered again" "200"
req GET "/tasks?leadId=$HOT_LEAD&size=50" "$MANAGER_TOKEN"
chk "no duplicate reminder after rescore (still 1)" "200"
REM_COUNT="$(printf '%s' "$BODY" | grep -o REMINDER | wc -l)"
CODE="200"; BODY="reminder count: $REM_COUNT"
chk "exactly one REMINDER task" "200" "reminder count: 1"
